...
function sendPrayerNotification(prayerName) {
	...
	try {
		...
		console.log(`Notification sent for ${prayerName}`);
	} catch (error) {
...
function checkPrayerTime() {
	...
	// Check if prayer time elements exist
	const fajrEl = document.querySelector('#fajr .time');
	if (!fajrEl || !fajrEl.textContent) {
	...
	for (const [name, time] of Object.entries(prayers)) {
		if (time && currentTime === time) {
			console.log(`Prayer time match found: ${name} at ${time}`);
